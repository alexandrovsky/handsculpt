...
{
	public static string PRIMARY_HAND = "Primary Hand";
	public static string SECONDARY_HAND = "Secondary Hand";
	public static string UNKNOWN_HAND = "UNKNOWN Hand";
...
//	public Vector3 m_LeapScaling = new Vector3(0.02f, 0.02f, 0.02f);
	public Vector3 m_LeapScaling = new Vector3(0.5f, 0.5f, 0.5f);
...
//			hands.transform.position = new Vector3(0.0f, -10.0f, 0.0f);
		}
		else
		{
		...
		{
			behavior.m_fingers[i] = CreateFinger(behavior.m_hands[2], i); // apply to unknown hand
		}
	...
	private GameObject CreateHand(GameObject parent, int index)
	{
		...
		if( index == 0 )
			hand.name = PRIMARY_HAND;
		else if( index == 1 )
			hand.name = SECONDARY_HAND;
		else
			hand.name = UNKNOWN_HAND;
