...
{	
	...
	public void SetCollidable( GameObject obj, bool collidable )
	{
		...
		*/
		foreach( Collider component in obj.GetComponents<Collider>() ){
			...
			//Debug.Log("Set" + component.ToString() + "Collidable "+ collidable.ToString() );
		}
	...
	public int HandIdForIndex(int index){
		return m_handIDs[index];
	}
...
//		updatePalm(h, m_palms[index]);
	...
	void updatePalm( Leap.Hand leapHand, GameObject palmObject )
	{
		...
		if( leapHand.IsValid )
		{
			...
			Vector3 palmNormal = leapHand.PalmNormal.ToUnityTranslated();
			palmObject.transform.localRotation = Quaternion.FromToRotation(Vector3.down, palmNormal);
